...
const ArtistPage: React.FC<ArtistPageProps> = ({ user, onLogout }) => {
  ...
  const loadArtistData = async (id: string) => {
    setLoading(true);
    try {
      // Load artist details and top tracks
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#121212' }}>
        ...
        <div style={{ 
          ...
          <div style={{ textAlign: 'center', color: 'white' }}>
            ...
            <p style={{ color: '#B3B3B3', margin: 0 }}>Getting the latest info</p>
          </div>
        </div>
      </div>
    );
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '24px', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ color: '#B3B3B3', lineHeight: '1.6', fontSize: '16px', marginTop: '16px' }}>
                  Their music often evokes {artist.popularity >= 90 ? 'energetic and uplifting' : artist.popularity >= 70 ? 'emotional and introspective' : 'contemplative and calming'} feelings, 
                  making them perfect for listeners seeking {getMoodFromPopularity(artist.popularity).mood} vibes.
                </p>
              </div>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ color: getMoodFromPopularity(artist.popularity).color, fontSize: '12px', margin: 0, textTransform: 'capitalize' }}>
                  {getMoodFromPopularity(artist.popularity).mood}
                </p>
              </div>
            </div>
          </div>
